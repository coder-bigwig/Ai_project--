...
if "%COMPOSE_PROJECT_NAME%"=="" (
  for /f %%I in ('powershell -NoProfile -Command "$n = Split-Path -Leaf (Get-Location); $n = $n.ToLower() -replace '[^a-z0-9_-]',''; if ([string]::IsNullOrWhiteSpace($n)) { $n = 'training' }; if ($n -notmatch '^[a-z0-9]') { $n = 'p' + $n }; Write-Output $n"') do set "COMPOSE_PROJECT_NAME=%%I"
)
