...
function AdminResourceControl({ username }) {
  ...
  const loadOverview = useCallback(async () => {
    ...
    try {
      ...
      setErrorMessage(error?.response?.data?.detail || '加载资源控制数据失败');
    } finally {
  ...
  const saveUserQuota = async (rowUsername) => {
    ...
    try {
      ...
      setInfoMessage(`已更新 ${rowUsername} 的资源配额`);
      await loadOverview();
      await loadLogs();
    } catch (error) {
      setErrorMessage(error?.response?.data?.detail || `更新 ${rowUsername} 配额失败`);
    } finally {
  ...
  const resetUserQuota = async (rowUsername) => {
    ...
    try {
      ...
      setInfoMessage(`已恢复 ${rowUsername} 的默认资源配额`);
      await loadOverview();
      await loadLogs();
    } catch (error) {
      setErrorMessage(error?.response?.data?.detail || `恢复 ${rowUsername} 默认配额失败`);
    } finally {
      setSavingUser((prev) => ({ ...prev, [rowUsername]: false }));
    }
  };
  ...
  const saveBudget = async () => {
    ...
    try {
      ...
      setErrorMessage(error?.response?.data?.detail || '更新服务器资源预算失败');
    } finally {
    ...
    <div className="admin-resource-panel">
      <div className="admin-resource-head">
        ...
        <div className="admin-resource-head-actions">
          <button type="button" onClick={loadOverview} disabled={loadingOverview}>刷新资源监控</button>
      ...
      <div className="admin-resource-summary-grid">
        <article>
          <h3>用户与容器</h3>
          <p>总用户：{summary.total_users ?? 0}</p>
          <p>学生：{summary.students ?? 0}</p>
          <p>教师：{summary.teachers ?? 0}</p>
          <p>管理员：{summary.admins ?? 0}</p>
                      ...
                      <tr key={item.username}>
                        ...
                        <td>
                          <span className={`status ${item.server_running ? 'running' : (item.server_pending ? 'pending' : 'stopped')}`}>
                            {item.server_running ? '运行中' : (item.server_pending ? '启动中' : '未运行')}
                          </span>
                        ...
                        <td>
                          ...
                          />
                          <div className="sub">
                            {item.quota_source === 'custom' ? `自定义：${item.quota_updated_by || '-'} ${formatDateTime(item.quota_updated_at)}` : '默认配额'}
                          </div>
                        </td>
